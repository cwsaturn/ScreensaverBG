Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/5edad83115ea2764a9e2ec60a437a419"
{
	Properties
	{
...
#define PROP_VERTEXROUNDINGDIVISION 0.02
...
#define PROP_SHADINGENABLED 0
#define PROP_LIGHTINGMODE 5
...
#define PROP_FLIPBOOKCURRENTFRAME 0
...
#define PROPM_END_STENCILPASSOPTIONS 0
			...
			void applyAlphaOptions(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiCam poiCam, in PoiMods poiMods)
			{
				...
				if (float(0))
				{
					...
					half3 modelForwardDirection = normalize(mul(unity_ObjectToWorld, normalize(float4(0,0,1,1).rgb)));
			...
			void ApplyAlphaToCoverage(inout PoiFragData poiFragData, in PoiMesh poiMesh)
			{
				...
				if (float(3) == 1)
				{
					...
					if (float(0) && float(0))
					{
						poiFragData.alpha *= 1 + max(0, CalcMipLevel(poiMesh.uv[0] * float4(1,1,1,1).zw)) * float(0.25);
